...
$queryCurrency = "SELECT `id` AS ind, `title` AS label
    FROM `currency`
    ORDER BY `rank`";


$ddCurrency = '<select name="currency_id" id="currency_id" style="width:300px"><option value="">Select Currency</option>';
$ddCurrency .= create_item_list($queryCurrency, $currency_id);
$ddCurrency .= '</select>';

$ddCurrency = <<< H
    <td><label for="currency_id">Select Currency:</label></td>
    <td>$ddCurrency</td>
H;


    $resbookVersionSQL = "SELECT `version` AS ind, `name` AS label
    FROM `resbook_version`
    ORDER BY `version`";
    $resbookVersionOptions = create_item_list($resbookVersionSQL, $resbook_version);
    ...
    $companydetails_content = <<< HTML
        <table width="100%" border="0" cellspacing="0" cellpadding="4">
            ...
            <tr>
                <td width="150"><label for="resbook_id">Resbook Version</label></td>
                <td>
                    <select name="resbook_version" id="resbook_version">
                    <option value="">-- Select ResBook Version --</option>
                    {$resbookVersionOptions}
                    </select>
                </td>
            </tr>
            <tr>
...
H;
    ...
    //Editable CTA Tab
    include_once('views/general_cta.php');
    ...
    $temp_array_menutab ['CTA Content']           = $contentCtaView;
...
function save_item()
{
    ...
    $temp_array_save['resbook_version']            = sanitize_input('resbook_version');
    ...
    $temp_array_save['currency_id']                = sanitize_input('currency_id');
    ...
    if(!empty($ctaItems)){
        ...
        foreach ($ctaItems as $ctaItemId => $ctaItemContent) {

            $tempCtaData = array();

            $tempCtaData['content']          = $ctaItemContent;

            update_row($tempCtaData, 'general_cta', "WHERE id = '{$ctaItemId}'");
    ...
    // preprint_r($_REQUEST['is_active']);
    // preprint_r(array_keys($_REQUEST['is_active']));
    // preprint_r(implode(',',array_keys($_REQUEST['is_active'])));

    if (isset($_REQUEST['is_active'])) {

        run_query("UPDATE `cms_module_group` SET `is_active` = 'N' WHERE `is_active` = 'Y'");
        
        $moduleIds = array_keys($_REQUEST['is_active']);

        $moduleIds = implode(',', $moduleIds);
        run_query("UPDATE `cms_module_group` SET `is_active` = 'Y' WHERE `cms_module_group_id` IN ({$moduleIds})");
