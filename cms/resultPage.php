...
function build_menu(){
    ...
    $parentNavSqlExt = (IS_SUPER_USER != 'Y') ? "WHERE cmg.`is_active` = 'Y'" : "";


    $parentNavModules = run_query("SELECT DISTINCT(cmg.`cms_nav_group_id`) AS `id`, cng.`cms_nav_label`
                FROM `cms_module_group` cmg
                LEFT JOIN `cms_nav_group` cng
                ON(cmg.`cms_nav_group_id` =cng.`cms_nav_id`)
                {$parentNavSqlExt}
                ORDER BY cng.`cms_nav_rank` ASC");

    $menuPages = array();

    if (mysql_num_rows($parentNavModules) > 0) {

    	while ($arrParentNavModule = mysql_fetch_assoc($parentNavModules)) {

            // preprint_r($arrParentNavModule);
            $arrMenuItemGroup = array();
            $grpModuleSqlExt = (IS_SUPER_USER != 'Y') ? "AND `is_active` = 'Y'" : "";

            $grpModules = fetch_value("SELECT GROUP_CONCAT(`cms_module_group_id`) FROM
                            `cms_module_group`
                            WHERE `cms_nav_group_id` = {$arrParentNavModule['id']}
                            {$grpModuleSqlExt}");

            $modules = fetch_all("SELECT `cms_module_id`,
                        `cms_module_label`,
                        `cms_module_uri`,
                        `cms_module_icon`,
                        `cms_module_rank`,
                        ...
                        `access_key`
                        FROM `cms_module`
                        WHERE `cms_module_group_id` IN ({$grpModules})
                        ORDER BY `cms_module_rank` ASC");

            $menuItem = array();

            foreach ($modules as $module) {
                $menuItem[] = array('label' => $module['cms_module_label'],
                                  'uri' => $module['cms_module_uri'],
                                  'icon_cls' => $module['cms_module_icon']);
            }

            $arrMenuItemGroup = array('group_label' => $arrParentNavModule['cms_nav_label'],
                                      'items' => $menuItem);

            array_push($menuPages, $arrMenuItemGroup);
    	}
    ...
    array_push($menuPages, $menu_pages);
