...
$inactiveModules = fetch_value("SELECT GROUP_CONCAT(m.`mod_id`) AS `mod_id`
							FROM  `modules` m
							LEFT JOIN `cms_module_group` cmg
							ON(m.`module_group_id` = cmg.`cms_module_group_id`)
							WHERE `is_active` = 'N'");

$inactiveModIds = explode(',', $inactiveModules);
$attachedModIds = array_extract(fetch_all($sql),'id');

$activeModules = array_diff($attachedModIds, $inactiveModIds);
$mod_path = array_extract(fetch_all($sql),'mod_path');
$mod_id = array_extract(fetch_all($sql),'id');

if(!empty($mod_path))
{
	foreach($mod_path as $key => $path)
	{
		if (in_array($mod_id[$key], $activeModules)) {
